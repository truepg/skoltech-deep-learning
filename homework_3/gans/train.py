...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            self.val_noise.data = torch.tensor(stats.truncnorm.rvs(-1, 1, size=self.val_noise.shape), dtype=torch.float)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        noise_shape = labels.shape[0], self.noise_channels
        noise = torch.randn(noise_shape)

        noise = noise.to(labels.device)

        real_scores = self.dis(imgs, labels)
        gen_out = self.forward(noise, labels)
        fake_scores = self.dis(gen_out, labels)

        if optimizer_idx == 0:
          loss = self.loss(fake_scores=fake_scores)
        else:
          loss = self.loss(fake_scores=fake_scores, real_scores=real_scores)
